...
class CidadaoKafkaResourceIT {
    ...
    void setup() {
        ...
        CidadaoKafkaResource kafkaResource = new CidadaoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/cidadao-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/cidadao-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
